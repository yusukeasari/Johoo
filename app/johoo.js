...
/* 外部設定予定 ここから */
(function () {
  var Browser, ClickOnlyButton, ControlPanel, ControlPanelModel, DT, DeleteValueButton, INIT_FILE, MID, Marker, PhotomosaicViewer, Point, Popup, PostPanel, Pyramid, SEARCH_API, SModel, SearchPanel, SearchResult, Shadow, SmallMap, TIMELINE_API, Tile, TileView, Tiles, Timeline, TimelineChild, TimelineChildView, UID, Utility, arrZoomSizeX, arrZoomSizeY, commentZoom, getSection, getUrlVars, initJson, initJsonPath, minBlockSize, minZoom, motifHeight, motifWidth, nowZoom, pinchTrigger, prevZoom, setInitData, tileHeight, tileImageDir, tileImageExtension, tileWidth, tlImageWidth, zoomImageDir, zoomSize, __bind = function (fn, me) {
      return function () {
  ...
  PhotomosaicViewer = function (_super) {
    ...
    function PhotomosaicViewer() {
      ...
      this.showSearchPanel = __bind(this.showSearchPanel, this);
      this.backtomain = __bind(this.backtomain, this);
      this.openPopupFromTimeline = __bind(this.openPopupFromTimeline, this);
      this.openPopupFromPoint = __bind(this.openPopupFromPoint, this);
      this.onOrient = __bind(this.onOrient, this);
      this.initialize = __bind(this.initialize, this);
      this.onMouseLeave = __bind(this.onMouseLeave, this);//追加)PCでモザイクをウィンドウ外へドラッグされた場合
    ...
    PhotomosaicViewer.prototype.initialize = function () {
      ...
      $('<link>').attr('href', css_href).attr('rel', 'stylesheet').load().appendTo($('head'));
		...
		$(this.el).bind('mouseleave', this.onMouseLeave);//追加)PCでモザイクをウィンドウ外へドラッグされた場合(※mouseoutイベントではない)
      return this.setup('');
    };
    PhotomosaicViewer.prototype.onMouseLeave = function(e){
    	// console.log('#johoo mouse leave');
    	$('#Tiles').trigger('mouseup');
    }
    ...
    PhotomosaicViewer.prototype.openPopupFromPoint = function (_id) {
      var p;
      p = 0;
      return $.getJSON(SEARCH_API, { 'id': _id }, function (_this) {
    ...
    PhotomosaicViewer.prototype.setup = function (_init) {
      ...
      this.searchPanel.bind('backtomain', function (_this) {
        return function () {
          return _this.router.navigate('', { trigger: true });
        };
      }(this));
      ...
      this.pyramid.bind('openPopupFromPoint', function (_this) {
        return function (_p) {
          return $.getJSON(SEARCH_API, { 'n': _p }, function (data, status) {
            var p;
            if (status && data !== null) {
              if (data[0].id !== void 0) {
                p = data[0].id;
                return _this.router.navigate('mosaic/' + p + '/', { trigger: true });
              }
            }
          }).fail(function () {
            return console.log('error:' + status);
      ...
      this.controlPanel.bind('showSearchPanel', function (_this) {
        return function () {
          return _this.router.navigate('search/', { trigger: true });
        };
      }(this));
      ...
      this.searchPanel.bind('onclicktimeline', function (_this) {
        return function (d) {
          return _this.router.navigate('timeline/' + d + '/', { trigger: true });
        };
      }(this));
      this.pyramid.bind('moving', function (_this) {
        return function (c) {
        };
      }(this));
      ...
      this.controlPanel.bind('onclickhomebutton', function (_this) {
        return function () {
          nowZoom = 2;//PCデバイスで初期表示修正する場合
          prevZoom = 3;
        	if(Browser.device!='pc'){
				nowZoom = 1;//1210初期表示修正
				prevZoom = 2;
        	}
      ...
      this.controlPanel.bind('showPostPanel', function (_this) {
        return function () {
  ...
  SmallMap = function (_super) {
    ...
    SmallMap.prototype.setup = function () {
      ...
      return this.setCoords([
        Browser.width / 2 - zoomSize[nowZoom][0] / 2,
        Browser.height / 2 - zoomSize[nowZoom][1] / 2
      ]);
    };
  ...
  SModel = function (_super) {
    ...
    function SModel() {
      this.cEvent = __bind(this.cEvent, this);
  ...
  PostPanel = function (_super) {
    ...
    function PostPanel() {
      ...
      this.show = __bind(this.show, this);
      this.initialize = __bind(this.initialize, this);
      return PostPanel.__super__.constructor.apply(this, arguments);
    }
  ...
  SearchPanel = function (_super) {
    ...
    function SearchPanel() {
      ...
      this.setup = __bind(this.setup, this);
      this.appendTimeline = __bind(this.appendTimeline, this);
      this.onclicktimeline = __bind(this.onclicktimeline, this);
      this.onBackToMain = __bind(this.onBackToMain, this);
      this.searchpanelloaded = __bind(this.searchpanelloaded, this);
      this.initialize = __bind(this.initialize, this);
    ...
    SearchPanel.prototype.initialize = function () {
      ...
      return $(this.el).load('./searchPanel.html', this.searchpanelloaded);
    };
    SearchPanel.prototype.searchpanelloaded = function (data, status) {
		...
		if ((appVersion.indexOf("msie 8.") != -1 || appVersion.indexOf("msie 9.") != -1)) {//userAgent.indexOf('msie') != -1　&&　
			// console.log("IE8 or IE9");
			// IE8,9フォームのplaceholder属性をjQueryで対応。
    ...
    SearchPanel.prototype.appendTimeline = function (tile) {
      ...
      // PC用スタイル変更のため各テキスト表示幅を継承させるように変更
      // $('.tlTitle').css({ width: Browser.width - tlImageWidth - 10 });
      // return $('.tlMsg').css({ width: Browser.width - tlImageWidth - 10 });
      $('.tlTitle').css({ width: $('#searchResult').width() - tlImageWidth - 10});
      return $('.tlMsg').css({ width: $('#searchResult').width() - tlImageWidth - 10});
    };
    ...
    SearchPanel.prototype.onTapSubmitButton = function () {
      $('#loadingAnimation').hide();//追加(検索タイムラインからの初期化)
      $('#loadingAnimation').height(0);//追加(検索タイムラインからの初期化)
    ...
    SearchPanel.prototype.sendQuery = function () {
      ...
      // IE8,9フォームのplaceholder対応。(そのままvalueとして送信されないようにするため)
      if($('#SearchPanelInnerContents #id').val()=="IDを検索(6ケタ)"){
		$('#SearchPanelInnerContents #id').val("");
      }
      if($('#SearchPanelInnerContents #b1').val()=="タイトルを検索"){
		$('#SearchPanelInnerContents #b1').val("");
      }
      if($('#SearchPanelInnerContents #b2').val()=="コメントを検索"){
		$('#SearchPanelInnerContents #b2').val("");
      }
      ...
      if ($('#SearchPanelInnerContents #b1').val() !== void 0) {
      	// IEで文字化けするのでencodeURI追加
        query += 'b1=' + encodeURIComponent($('#SearchPanelInnerContents #b1').val()) + '&';
      }
      if ($('#SearchPanelInnerContents #b2').val() !== void 0) {
        query += 'b2=' + encodeURIComponent($('#SearchPanelInnerContents #b2').val()) + '&';
      }
      if ($('#SearchPanelInnerContents #b3').val() !== void 0) {
        query += 'b3=' + encodeURIComponent($('#SearchPanelInnerContents #b3').val()) + '&';
      }      
      if (query !== '') {
        query.slice(0, -1);
      }
    ...
    SearchPanel.prototype.render = function (result) {
      ...
      //追加(検索ボタンタップ後、検索タイムライン上に表示)
      $('#loadingAnimation').show();

      switch (ERROR) {
      case 'TOOMUCHRESULT':
        // this.error('<br />\u691c\u7d22\u7d50\u679c\u304c100\u4ef6\u3092\u8d85\u3048\u307e\u3057\u305f\u3002<br />\u6761\u4ef6\u3092\u6307\u5b9a\u3057\u306a\u304a\u3057\u3066\u304f\u3060\u3055\u3044\u3002');
        this.error('<br />\u691c\u7d22\u7d50\u679c\u304c300\u4ef6\u3092\u8d85\u3048\u307e\u3057\u305f\u3002<br />\u6761\u4ef6\u3092\u6307\u5b9a\u3057\u306a\u304a\u3057\u3066\u304f\u3060\u3055\u3044\u3002');
        break;
      case 'NOTFOUND':
    ...
    DeleteValueButton.prototype.initialize = function (_el) {
      ...
      this.el.children('input').bind('keyup', function (_this) {
        return function () {
          if (_this.el.children('input').val() === '') {
            return $('#' + _this.el.children('input').attr('id') + 'DelButton').css({ 'opacity': 0 });
          } else {
            return $('#' + _this.el.children('input').attr('id') + 'DelButton').css({ 'opacity': 1 });
          }
        };
      }(this));
      $('#' + this.el.children('input').attr('id') + 'DelButton').bind('click', function (_this) {
        return function () {
          ...
          $('#' + _this.el.children('input').attr('id') + 'DelButton').css({ 'opacity': 0 });
          return $('#' + _this.el.children('input').attr('id') + 'DelButton').focus();
        };
      }(this));
      return $('#' + this.el.children('input').attr('id') + 'DelButton').css({ 'opacity': 0 });
    };
  ...
  TimelineChild = function (_super) {
    ...
    TimelineChild.prototype.defaults = { data: '' };
  ...
  TimelineChildView = function (_super) {
    ...
    TimelineChildView.prototype.events = { 'click': 'onclicks' };
  ...
  SearchResult = function (_super) {
    ...
    SearchResult.prototype.queryResult = function (result) {
    	// IE8,9placehoder対応追加(プラグインah-placehoder.js用イベント)
    	$('[placeholder]').trigger('return', result);
  ...
  Pyramid = function (_super) {
    ...
    function Pyramid() {
      ...
      this.getPyramidPos = __bind(this.getPyramidPos, this);
      this.getMousePos = __bind(this.getMousePos, this);
      this.pyramidSetPositionToCenter = __bind(this.pyramidSetPositionToCenter, this);
      this.appendTile = __bind(this.appendTile, this);
      this.convertToLocalCenterPos = __bind(this.convertToLocalCenterPos, this);
      this.convertToGrobalCenterPos = __bind(this.convertToGrobalCenterPos, this);
      this.moveToZoomOutPos = __bind(this.moveToZoomOutPos, this);
      this.moveToZoomInPos = __bind(this.moveToZoomInPos, this);
      this.moveToPinchZoomPos = __bind(this.moveToPinchZoomPos, this);
      this.moveToNum = __bind(this.moveToNum, this);
      this.update = __bind(this.update, this);
      this.render = __bind(this.render, this);
      this.checkActiveTile = __bind(this.checkActiveTile, this);
      this.getNumFromPoint = __bind(this.getNumFromPoint, this);
      this.isSingleTap = __bind(this.isSingleTap, this);
      this.isOnTiles = __bind(this.isOnTiles, this);
      this.zoomOut = __bind(this.zoomOut, this);
      this.zoomIn = __bind(this.zoomIn, this);
      this.onGestureEnd = __bind(this.onGestureEnd, this);
      this.onGestureMove = __bind(this.onGestureMove, this);
      this.onGestureStart = __bind(this.onGestureStart, this);
    ...
    Pyramid.show = function () {
      return $(Pyramid.outerel).show();
    };
    Pyramid.hide = function () {
      return $(Pyramid.outerel).hide();
    };
    /*
    	マウスイベント関連メソッド群
     */
    ...
    Pyramid.prototype.onMouseUp = function (e) {
    	// console.log('Tiles mouse up');//debug用
      ...
      if (cords !== void 0 && Point.isLock() === false) {
        ...
        if (this.isSingleTap(this.dragStartX, cordx) && this.isSingleTap(this.dragStartY, cordy)) {
          if (!Shadow.isShow() && nowZoom > 3) {
            ...
            return this.trigger('openPopupFromPoint', this.getNumFromPoint([
              cords[0],
              cords[1]
            ]));
          }
        } else if (this.isSingleTap(this.dragStartX, cordx) && this.isSingleTap(this.dragStartY, cordy) && this.isOnTiles([
            cords[0][0],
            cords[0][1]
          ])) {
          if (!Shadow.isShow() && nowZoom > 3) {
    ...
    Pyramid.prototype.onMouseMove = function (e) {
      ...
      if (cords !== void 0 && Point.isLock() === false) {
        ...
        if (Utility.type(cords[0]) === 'number' && this.dragging === true) {
          ...
          return this.trigger('moving', [
            this.dragStartPyramidX + (this.getMousePos(e)[0] - this.dragStartX),
            this.dragStartPyramidY + (this.getMousePos(e)[1] - this.dragStartY)
          ]);
    ...
    Pyramid.prototype.onGestureStart = function (e) {
      if (Point.isLock() === false) {
        return $(this.el).css({ transform: 'scale(1)' });
      }
    };
    ...
    Pyramid.prototype.moveToNum = function (d) {
      ...
      return this.trigger('moving', [
        Browser.width / 2 + tx + arrZoomSizeX[nowZoom] / 2,
        Browser.height / 2 + ty - arrZoomSizeY[nowZoom] / 2
      ]);
    };
  ...
  TileView = function (_super) {
    ...
    TileView.prototype.render = function () {
      ...
      y = this.model.get('y');
      z = this.model.get('z');
      url = tileImageDir + ('' + z + '/' + y + '/') + 'z' + z + 'x' + x + 'y' + y + tileImageExtension+ '?' + Utility.getRandom();// + '?' + Utility.getRandom()//リアルタイム更新の場合
      $(this.el).attr({
        id: 'z' + z + 'x' + x + 'y' + y,
        src: url
      }).css({
  ...
  Tiles = function (_super) {
    ...
    function Tiles() {
      this.removeAllTiles = __bind(this.removeAllTiles, this);
      this.removeCheckedTiles = __bind(this.removeCheckedTiles, this);
      this.setRemove = __bind(this.setRemove, this);
      this.getNowVisibleList = __bind(this.getNowVisibleList, this);
      this.isSameTile = __bind(this.isSameTile, this);
      this.initialize = __bind(this.initialize, this);
  ...
  ControlPanel = function (_super) {
    ...
    function ControlPanel() {
      this.showPostPanel = __bind(this.showPostPanel, this);
      this.onclickhomebutton = __bind(this.onclickhomebutton, this);
      this.showSearchPanel = __bind(this.showSearchPanel, this);
      this.zoomOut = __bind(this.zoomOut, this);
      this.zoomIn = __bind(this.zoomIn, this);
      this.initialize = __bind(this.initialize, this);
  ...
  Point = function () {
    ...
    Point.getPoint = function (e) {
      var cords, ftime, hx, hy, item, lx, ly, _i, _len, _ref;
      // console.log("e.originalEvent.touches:"+e.originalEvent);//この時点ではtouchesはあるかないか不明
      if (Point.isTouch()) {
    ...
    Point.isTouch = function () {
    	// console.log("'ontouchstart' in window:"+('ontouchend' in document));
    	var isTouch=false;
    	isTouch = (Browser.device != 'pc');
    	/*
    	Windows8.1のChromeで('ontouchstart' in window)->trueを返すようになったため。
    	*/
      return isTouch;
    };
    return Point;
  ...
  Popup = function (_super) {
    ...
    function Popup() {
      ...
      this.hide = __bind(this.hide, this);
      this.show = __bind(this.show, this);
      this.mosaicButtonAction = __bind(this.mosaicButtonAction, this);
    ...
    Popup.prototype.openPopupFromPoint = function (p) {
      Shadow.show();
      return $.getJSON(SEARCH_API, { 'n': p }, function (_this) {
        return function (data, status) {
          if (status && data !== null) {
    ...
    Popup.prototype.render = function (data) {
      ...
      return $('<img />').css('margin-top', 5).attr('src', zoomImageDir + data.img + tileImageExtension).load(function (_this) {
        return function () {
          var vf, vt, vt2;
          vf = encodeURIComponent('http://pearl.pitcom.jp/#mosaic/' + data.id+'/');
          vt = encodeURIComponent("http://pearl.pitcom.jp/#mosaic/" + data.id+'/');
          vt2 = encodeURIComponent('PITビューアデモ');
    ...
    Popup.prototype.snsButtonAction = function (_id) {
      $('.snsFacebookButton').bind('touchend', function (_this) {
        return function (e) {
        	// _gaqオブジェクトはユニバーサルアナリティクスでは非対応->undefined
        	/*
          _gaq.push([
            '_trackPageview',
            '/photomosaic/sp/fb/' + _id
          ]);
*/
          return $('.snsFacebookButton').unbind();
        };
      }(this));
      $('.snsTwitterButton').bind('touchend', function (_this) {
        return function (e) {
        	/*
          _gaq.push([
            '_trackPageview',
            '/photomosaic/sp/tw/' + _id
          ]);
*/
          return $('.snsTwitterButton').unbind();
        };
      }(this));
      return $('.snsLineButton').bind('touchend', function (_this) {
        return function (e) {
        	/*
          _gaq.push([
            '_trackPageview',
            '/photomosaic/sp/line/' + _id
          ]);
*/
    ...
    Popup.prototype.mosaicButtonAction = function (_mid) {
      $('#mosaicButton').bind('touchend', function (_this) {
        return function (e) {
          location.href = 'mosaicView.php?mid=' + _mid;
          return $('#mosaicButton').unbind();
        };
      }(this));
      return $('#mosaicButton').bind('mouseup', function (_this) {
        return function (e) {
          location.href = 'mosaicView.php?mid=' + _mid;
          return $('#mosaicButton').unbind();
        };
  ...
  setInitData = function (data) {
    ...
    for (_i = 0, _len = arrZoomSizeX.length; _i < _len; _i++) {
      x = arrZoomSizeX[_i];
      zoomSize.push([
        motifWidth * minBlockSize * arrZoomSizeX[i],
        motifHeight * minBlockSize * arrZoomSizeY[i]
      ]);
      i++;
    }
  ...
  $(window).load(function (_this) {
    return function () {
      return getSection(initJsonPath, setInitData);
    };
  }(this));
}.call(this));